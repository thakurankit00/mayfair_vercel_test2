...
/*
INSERT INTO room_images (room_type_id, image_url, image_path, original_filename, file_size, mime_type, width, height, is_primary, display_order, alt_text)
SELECT 
    rt.id,
    '/uploads/rooms/sample-' || rt.name || '.jpg',
    'uploads/rooms/sample-' || rt.name || '.jpg',
    'sample-' || rt.name || '.jpg',
    1024000,
    'image/jpeg',
    ...
    rt.name || ' room image'
FROM room_types rt
WHERE NOT EXISTS (SELECT 1 FROM room_images WHERE room_type_id = rt.id);
*/
